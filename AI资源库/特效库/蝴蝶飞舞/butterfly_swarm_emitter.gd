...
const FX_TEX := preload("res://AI资源库/特效库/蝴蝶飞舞/fx.png")
const COLS := 7
const MODE := "particles"
const GIZMO_COLOR := Color(0.550, 0.600, 1.000, 0.160)
const POINT_LIGHT_COLOR := Color(1.000, 1.000, 1.000, 1.000)

@export var spawn_rect_half_extents: Vector2 = Vector2(58, 34):
	set(v):
		spawn_rect_half_extents = v
		queue_redraw()

@export_range(0.0, 180.0, 0.5) var particles_per_second: float = 3.5
@export_range(0.05, 10.0, 0.01) var particle_lifetime: float = 2.2
@export var velocity_min: Vector2 = Vector2(-18, -10)
@export var velocity_max: Vector2 = Vector2(18, 8)
...
@export_range(0.05, 4.0, 0.01) var particle_scale_min: float = 0.7
@export_range(0.05, 4.0, 0.01) var particle_scale_max: float = 1.1
@export_range(0.05, 4.0, 0.01) var single_scale: float = 0.7
@export_range(0.0, 24.0, 0.1) var jitter_pixels: float = 1.2
@export var random_rotation: bool = true
@export var spin_speed_range: Vector2 = Vector2(-0.4, 0.4)
