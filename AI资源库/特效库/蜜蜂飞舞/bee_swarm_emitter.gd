...
const FX_TEX := preload("res://AI资源库/特效库/蜜蜂飞舞/fx.png")
const COLS := 7
const MODE := "particles"
const GIZMO_COLOR := Color(1.000, 0.840, 0.200, 0.160)
const POINT_LIGHT_COLOR := Color(1.000, 1.000, 1.000, 1.000)

@export var spawn_rect_half_extents: Vector2 = Vector2(54, 28):
	set(v):
		spawn_rect_half_extents = v
		queue_redraw()

@export_range(0.0, 180.0, 0.5) var particles_per_second: float = 5.0
@export_range(0.05, 10.0, 0.01) var particle_lifetime: float = 1.8
@export var velocity_min: Vector2 = Vector2(-14, -6)
@export var velocity_max: Vector2 = Vector2(14, 6)
@export_range(-200.0, 300.0, 0.5) var gravity: float = 0.0
@export_range(1.0, 60.0, 0.5) var animation_fps: float = 14.0:
	set(v):
		animation_fps = v
...
@export_range(0.05, 4.0, 0.01) var single_scale: float = 0.65
@export_range(0.0, 24.0, 0.1) var jitter_pixels: float = 1.6
@export var random_rotation: bool = true
@export var spin_speed_range: Vector2 = Vector2(-0.6, 0.6)
