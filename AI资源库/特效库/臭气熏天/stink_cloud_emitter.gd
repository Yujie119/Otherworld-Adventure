...
const FX_TEX := preload("res://AI资源库/特效库/臭气熏天/fx.png")
const COLS := 7
const MODE := "particles"
const GIZMO_COLOR := Color(0.450, 0.550, 0.140, 0.180)
const POINT_LIGHT_COLOR := Color(1.000, 1.000, 1.000, 1.000)

@export var spawn_rect_half_extents: Vector2 = Vector2(42, 16):
	set(v):
		spawn_rect_half_extents = v
		queue_redraw()

@export_range(0.0, 180.0, 0.5) var particles_per_second: float = 2.6
@export_range(0.05, 10.0, 0.01) var particle_lifetime: float = 2.7
@export var velocity_min: Vector2 = Vector2(-6, -15)
@export var velocity_max: Vector2 = Vector2(6, -5)
@export_range(-200.0, 300.0, 0.5) var gravity: float = -0.5
@export_range(1.0, 60.0, 0.5) var animation_fps: float = 10.0:
	set(v):
		animation_fps = v
		if _sprite_frames != null:
			_sprite_frames.set_animation_speed(&"default", animation_fps)

@export_range(0.05, 4.0, 0.01) var particle_scale_min: float = 0.78
@export_range(0.05, 4.0, 0.01) var particle_scale_max: float = 1.08
@export_range(0.05, 4.0, 0.01) var single_scale: float = 0.78
@export_range(0.0, 24.0, 0.1) var jitter_pixels: float = 0.8
@export var random_rotation: bool = false
@export var spin_speed_range: Vector2 = Vector2(-0.15, 0.15)
@export var fx_z_index: int = 7
