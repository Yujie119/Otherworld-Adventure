...
const FX_TEX := preload("res://AI资源库/特效库/鱼儿跳跃/fx.png")
const COLS := 7
const MODE := "particles"
const GIZMO_COLOR := Color(0.250, 0.700, 1.000, 0.140)
const POINT_LIGHT_COLOR := Color(1.000, 1.000, 1.000, 1.000)

@export var spawn_rect_half_extents: Vector2 = Vector2(36, 12):
	set(v):
		spawn_rect_half_extents = v
		queue_redraw()

@export_range(0.0, 180.0, 0.5) var particles_per_second: float = 0.75
@export_range(0.05, 10.0, 0.01) var particle_lifetime: float = 1.3
@export var velocity_min: Vector2 = Vector2(-2, 0)
@export var velocity_max: Vector2 = Vector2(2, 0)
...
@export_range(0.05, 4.0, 0.01) var particle_scale_min: float = 0.75
@export_range(0.05, 4.0, 0.01) var particle_scale_max: float = 1.0
@export_range(0.05, 4.0, 0.01) var single_scale: float = 0.75
