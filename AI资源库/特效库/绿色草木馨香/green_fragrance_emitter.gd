...
const FX_TEX := preload("res://AI资源库/特效库/绿色草木馨香/fx.png")
const COLS := 7
const MODE := "particles"
const GIZMO_COLOR := Color(0.350, 1.000, 0.350, 0.140)
const POINT_LIGHT_COLOR := Color(1.000, 1.000, 1.000, 1.000)

@export var spawn_rect_half_extents: Vector2 = Vector2(40, 18):
	set(v):
		spawn_rect_half_extents = v
		queue_redraw()

@export_range(0.0, 180.0, 0.5) var particles_per_second: float = 3.2
@export_range(0.05, 10.0, 0.01) var particle_lifetime: float = 2.4
@export var velocity_min: Vector2 = Vector2(-6, -12)
@export var velocity_max: Vector2 = Vector2(6, -4)
@export_range(-200.0, 300.0, 0.5) var gravity: float = -0.8
...
@export_range(0.05, 4.0, 0.01) var particle_scale_min: float = 0.32
@export_range(0.05, 4.0, 0.01) var particle_scale_max: float = 0.56
@export_range(0.05, 4.0, 0.01) var single_scale: float = 0.42
@export_range(0.0, 24.0, 0.1) var jitter_pixels: float = 0.5
@export var random_rotation: bool = true
@export var spin_speed_range: Vector2 = Vector2(-0.55, 0.55)
