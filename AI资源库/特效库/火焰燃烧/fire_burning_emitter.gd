...
const FX_TEX := preload("res://AI资源库/特效库/火焰燃烧/fx.png")
const COLS := 7
const MODE := "light"
const GIZMO_COLOR := Color(1.000, 0.450, 0.100, 0.160)
const POINT_LIGHT_COLOR := Color(1.000, 0.520, 0.120, 1.000)

@export var spawn_rect_half_extents: Vector2 = Vector2(32, 32):
	set(v):
...
@export_range(0.05, 4.0, 0.01) var particle_scale_min: float = 0.86
@export_range(0.05, 4.0, 0.01) var particle_scale_max: float = 0.86
@export_range(0.05, 4.0, 0.01) var single_scale: float = 0.95
...
@export_range(0.0, 3.0, 0.01) var light_energy: float = 0.62
@export_range(0.1, 5.0, 0.01) var light_texture_scale: float = 1.3
