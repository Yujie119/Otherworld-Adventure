...
func _ready() -> void:
	...
	var regions_enabled_now: bool = _runtime_regions_enabled()
...
func _update_runtime_actors() -> void:
	for key in _actors.keys():
		...
		var collision_hit: Dictionary = _actor_collision_hit_info(actor, collision_records) if is_primary else {}
...
func _create_adjust_ghost_visual(visual: CanvasItem) -> CanvasItem:
	...
	if parent == null:
		return null
	var clone_node: Node = visual.duplicate(0)
